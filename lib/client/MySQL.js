...
SQLDeleteHandler.prototype = Object.create(SQLHandler.prototype, {
	doPrepare: { value:
		function(data) {
			...
			if (!('table' in data)) {
				throw new Error('SQLDeleteHandler "' + handler.url + '" has no table name.');
			}
			...
			if (property.length) {
				...
				throw new Error('SQLDeleteHandler "' + handler.url + '" has where clause.');
			}
		}
	},
});
...
SQLInsertHandler.prototype = Object.create(SQLHandler.prototype, {
	doPrepare: { value:
		function(data) {
			...
			if (!('table' in data)) {
				throw new Error('SQLInsertHandler "' + handler.url + '" has no table name.');
			}
			if (!('columns' in data)) {
				throw new Error('SQLInsertHandler "' + handler.url + '" has no columns.');
			}
			for (var i = 0; i < data.columns.length; i++) {
				var column = data.columns[i];
				if (query.hasOwnProperty(column)) {
...
SQLSelectHandler.prototype = Object.create(SQLHandler.prototype, {
	doPrepare: { value:
		function(data) {
			...
			if (!('table' in data)) {
				throw new Error('SQLSelectHandler "' + handler.url + '" has no table name.');
			}
			if (!('columns' in data)) {
				throw new Error('SQLSelectHandler "' + handler.url + '" has no columns.');
			}
			for (var i = 0; i < data.columns.length; i++) {
				var column = data.columns[i];
				if (query.hasOwnProperty(column)) {
...
SQLUpdateHandler.prototype = Object.create(SQLHandler.prototype, {
	doPrepare: { value:
		function(data) {
			...
			if (!('table' in data)) {
				throw new Error('SQLUpdateHandler "' + handler.url + '" has no table name.');
			}
			if (!('columns' in data)) {
				throw new Error('SQLUpdateHandler "' + handler.url + '" has no columns.');
			}
			if (!('id' in query)) {
				throw new Error('SQLUpdateHandler "' + handler.url + '" has no id.');
			}
			for (var i = 0; i < data.columns.length; i++) {
				var column = data.columns[i];
				if (query.hasOwnProperty(column)) {
