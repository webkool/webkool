...
Application.prototype = Object.create(Object.prototype, {
	debugging: { value: false, writable: true, },
	filters: { value: {}, writable: true, },
	handlerConstructors: { value: {}, writable: true, },
	properties: { value: {port: 8080}, writable: true, },
	templateConstructors: { value: {}, writable: true, },
	warning: { value: true, writable: true, },
	...
	findHandler: { value:
		function(url, query) {
			...
			if (!constructor) {
				application.error("Handler not found '" + url + "'");
	...
	start: { value:
		function() {
			if (gHTTPServer) {
				...
				application.debug('server listening on port: ' + this.properties.port);
	...
	stop: { value:
		function() {
			if (!gHTTPServer && this.httpServer) {
				application.debug('stop listening on port: ' + this.properties.port);
...
var Error404Handler = Handler.template({
	...
	Behavior: Behavior.template ({
		on_render: function(handler, scope) {
			application.error("404 Not found: " + handler.url);
			if (handler._response) {
	...
	getMIME: { value:
		function (path) {
			...
			if (dot) {
				...
				application.warn('# mime not found!' + path + '\n');
			}
			return '';
		}
	},
	...
	parseDate: { value:
		function(d) {
			if (d) {
				...
				application.warn('# Date format was not recognize: ' + d + '\n');
			}
			return new Date();
		}
	},
