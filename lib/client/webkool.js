...
Application.prototype = Object.create(Object.prototype, {
	...
	sendErrorToResponse: { value:
		function(response, url, message) {
			...
			html += '</p></body></html>';
			response._response.writeHead(500, { 'Content-Type': 'text/html', 'Content-Length': html.length });
			response._response.end(html);
		}
	},
	request: { value:
		function(url, it) {
	...
	behavior: Behavior.template ({
		on_render: function(handler, model, query) {
			...
			if (handler._response) {
				...
				handler._response.writeHead(this.status, { 'Content-Type': 'text/html', 'Content-Length': html.length });
